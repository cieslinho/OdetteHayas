...
add_action( 'wp_enqueue_scripts', 'odette_scripts');

/**
 * Add admin styles and scripts
 */
function odette_admin_style() {
	wp_enqueue_style( 'main-admin-style', get_template_directory_uri() . '/assets/css/admin.css', [], false, 'all' );
}
add_action( 'admin_enqueue_scripts', 'odette_admin_style' );


function odette_config(){
  register_nav_menus(
    array(
  'odette_main_menu' => 'odette Main Menu',
    )
  );
}

add_action('after_setup_theme', 'odette_config', 0);
